...
export const parseCurrentPath = (currentPath: string) => {
  const arrayIndexes: number[] = [];
  const normalizedSegments = currentPath.split("/").map((segment) => {
    if (!INTEGER_SEGMENT.test(segment)) return segment;
 ...
 */
export const getArrayEntryPoint = <C extends PageConfigMap>(
  ...
  if (!addTransition?.target) return undefined;
...
export type ArrayInfoCache<C extends PageConfigMap> = Partial<
  Record<NodeKey<C>, ArrayInfo<C>>
>;
 ...
 */
export const buildArrayInfoCache = <C extends PageConfigMap>(
  pages: C,
  transitions: TransitionConfigMap<C>,
): ArrayInfoCache<C> => {
  const cache: ArrayInfoCache<C> = {};

  for (const [key, pageNode] of Object.entries(pages)) {
    const nodeKey = key as NodeKey<C>;
    if (!pageNode.arraySummary) continue;
    ...
    const arrayInfo: ArrayInfo<C> = { name: pageNode.arraySummary.name };
